...
void adjustBrick()  //.... GOD MODE
{
	...
	{
		delta = (movY+ (resY / 2) + 10) - (((resY / 2)) + obstacleBrick[i].y);
		if (delta < 0)
			delta *= -1;
		if ((movY + (resY / 2) + 10) <= (((resY / 2)) + obstacleBrick[i].y))
		{
...
void hitDetection()
{
	if (gameover == 0)
	{
		for (i = 0; i < obstacleBrick.size(); i++)
		{
			...
			{
				insidebrick = 1;
				//Bottom Brick Y original=(movY+resY/2) 				Top Brick Y	 original=(movY+resY/2+25)				                           
				if ((movY + (resY / 2) + 10) >= (((resY / 2) - 38) + obstacleBrick[i].y) && (movY + (resY / 2) + 30 - 5) <= (((resY / 2) + 38) + obstacleBrick[i].y))
				{
					hit1++;
				}
				else
